...
def max_numel(comm_op, dtype, mem_factor, args):
    ...
    max_memory_per_gpu = jax.local_devices()[0].memory_stats()['bytes_limit'] * mem_factor
    if comm_op == 'all_reduce' or comm_op == 'pt2pt' or comm_op == 'broadcast':
        elements_per_gpu = int(max_memory_per_gpu // dtype_size)
    elif comm_op == 'all_gather':
        elements_per_gpu = int(max_memory_per_gpu // dtype_size // jax.device_count())
        elements_per_gpu = int(pow(2, int(math.log(elements_per_gpu, 2))))
    elif comm_op == 'all_to_all':
        elements_per_gpu = int(max_memory_per_gpu // dtype_size)
        elements_per_gpu = int(jax.device_count() * round(elements_per_gpu / jax.device_count()))
        elements_per_gpu = int(pow(2, int(math.log(elements_per_gpu, 2))))
    else:
