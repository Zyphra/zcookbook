...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_partitions", type=int, default=1, help="Number of partitions to split the dataset into")
    ...
    n_proc = min(args.n_proc, args.partitions)
    inds_distr = more_itertools.distribute(n_proc, range(args.partitions))
    processes = []
    for process_inds in inds_distr:
        p = mp.Process(target=save_shard, args=(ds, args.save_path, args.partitions, process_inds))
        processes.append(p)
        p.start()
    for p in processes:
        p.join()
